...
42 EXISTING_PARTIAL Master Audit IDs through the server-log migration,
...
# ============================================================
# Expected 42 EXISTING_PARTIAL IDs through Phase 8
# ============================================================
PARTIAL_RULE_IDS = {
    2, 5, 10, 12, 13, 16, 17, 18, 19, 20, 22, 23, 25,
    28, 31, 32, 33, 34, 35, 37, 38, 39, 40, 43, 44, 47, 49, 50, 51, 52, 59, 60, 61,
    66, 67, 70, 74, 75, 76, 77, 78, 79,
}


class TestPartialRuleSet:
    """Verify the 42 EXISTING_PARTIAL rule set from CSV source of truth."""

    def test_csv_has_exactly_42_existing_partial(self):
        """CSV must contain exactly 42 EXISTING_PARTIAL rows."""
        ...
        assert len(partial_ids) == 42, (
            f"Expected 42 EXISTING_PARTIAL, got {len(partial_ids)}: {partial_ids}"
        )
    ...
    def test_partial_rule_ids_fixed_cardinality(self):
        """The expected set must have exactly 42 unique IDs."""
        assert len(PARTIAL_RULE_IDS) == 42
    ...
    def test_registry_matches_csv_partial_set(self):
        """The registry must have the same 42 EXISTING_PARTIAL IDs as CSV."""
