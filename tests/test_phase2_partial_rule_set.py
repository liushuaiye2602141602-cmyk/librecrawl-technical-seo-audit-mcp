...
39 EXISTING_PARTIAL Master Audit IDs (including GSC and Semrush migrations),
...
# ============================================================
# Expected 39 EXISTING_PARTIAL IDs through Phase 6
# ============================================================
PARTIAL_RULE_IDS = {
    2, 5, 10, 12, 13, 16, 17, 18, 19, 20, 22, 23, 25,
    28, 31, 32, 37, 38, 39, 40, 43, 44, 47, 49, 50, 51, 52, 59, 60, 61,
    66, 67, 70, 74, 75, 76, 77, 78, 79,
}


class TestPartialRuleSet:
    """Verify the 39 EXISTING_PARTIAL rule set from CSV source of truth."""

    def test_csv_has_exactly_39_existing_partial(self):
        """CSV must contain exactly 39 EXISTING_PARTIAL rows."""
        ...
        assert len(partial_ids) == 39, (
            f"Expected 39 EXISTING_PARTIAL, got {len(partial_ids)}: {partial_ids}"
        )
    ...
    def test_partial_rule_ids_fixed_cardinality(self):
        """The expected set must have exactly 39 unique IDs."""
        assert len(PARTIAL_RULE_IDS) == 39
    ...
    def test_registry_matches_csv_partial_set(self):
        """The registry must have the same 39 EXISTING_PARTIAL IDs as CSV."""
